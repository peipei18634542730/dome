...
public final int getAnddAddInt(Object var1,long var2,int var4){
    ...
    //compareAndSwapInt 中 var1和var2得到主内存的快照值，如果var5和这个值相等则，var5+var4
    return var5;
}
```
...
`HashMap`不是线程安全的，`Hashtable`是线程安全的，但是跟`Vector`类似，太重量级。所以也有类似CopyOnWriteMap，只不过叫`ConcurrentHashMap底层用synchronized修饰）`。
...
5. **绑定多个条件**：`sync`不能，只能随机唤醒。而`Lock`可以通过`Condition`来绑定多个条件，精确唤醒，`sync`使用Object对象本身的wait 、notify、notifyAll调度机制，而Lock可以使用Condition进行线程之间的调度。
...
![](E:\笔记\面试总结\imgs/BlockingQueue.png)
...
**API**：抛出异常是指当队列满时，再次插入会抛出异常；返回布尔是指当队列满时，再次插入会返回false；阻塞是指当队列满时，再次插入会被阻塞，直到队列取出一个元素，才能插入。超时是指当一个时限过后，才会插入或者取出。如列：
...
![threadPool](E:\笔记\面试总结\imgs\threadPool.png)

**流程图**：

![threadPoolProcedure](E:\笔记\面试总结\imgs\threadPoolProcedure.png)
...
对于CPU密集型任务，最大线程数是CPU线程数+1。对于IO密集型任务，尽量多配点，可以是CPU线程数*2，或者CPU线程数/(1-阻塞系数)。阻塞系数一般在0.8-0.9之间。
...
class HoldLockThread implements Runnable{
    ...
    public HoldLockThread(String lockA, String lockB) {//构造器
        this.lockA = lockA;
        this.lockB = lockB;
    }
...
==================================执行结果
线程B	 自己持有锁lockB尝试获得lockA
线程A	 自己持有锁lockA尝试获得lockB
    。。。。。。。 程序卡住，进入死锁状态
```
